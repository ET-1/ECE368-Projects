...
//Helper function to destory data structure created
...
//Function to free memory created by data sturcture
...
//reversing route to destination from source to make it able to print in order
...
//Printing route from source node to destination node
void finalPrint(VertixNodeList * sourceNode)
{
  if(sourceNode != NULL)
    {
      printf("%d ",sourceNode->vertix);
      sourceNode = sourceNode->prev;
      finalPrint(sourceNode);
    }
}

//resetting all visited nodes and all nodes affected by program to allow for next query to be run
